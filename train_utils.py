...
def init_spixel_grid(args,  b_train=True, downsize=16):
    if b_train:
    ...
    n_spixl_h = int(np.floor(img_height / downsize))   # args.downsize
    ...
    torch_spix_idx_tensor = torch.tensor(
                np.tile(spix_idx_tensor, (args.batch_size, 1, 1, 1))).cuda()
    
    # torch_spix_idx_tensor = torch_spix_idx_tensor.cuda()
...
# ============ accumulate Q =============================
def spixlIdx(args, b_train=False, downsize=16):
    # code modified from ssn
    if b_train:
        n_spixl_h = int(np.floor(512 / downsize))       # downsize
        n_spixl_w = int(np.floor(512 / downsize))
    else:
        n_spixl_h = int(np.floor(512 / downsize))       # downsize
        n_spixl_w = int(np.floor(512 / downsize))
